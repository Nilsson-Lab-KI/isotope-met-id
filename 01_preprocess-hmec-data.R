...
qc_peaks_path <- file.path(input_data_path, "qc-peaks-with-flags.tsv")
qc_peaks <- read.table(
    qc_peaks_path,
    sep = "\t", header = TRUE
) %>% mutate(peak_id = as.character(peak_id))

# TODO: see if we actually need this, when using HMDB mapping instead
...
hmec_peak_areas <- hmec_peak_areas[
    which(hmec_peak_areas$peak_id %in% qc_peaks$peak_id),
    column_order
]
